 ...
 * customer's balance, what the bank actually received, and what is still held
 * as an advance. Collapsing them into one figure is how a withholding
 * deduction becomes invisible.
 */
export default function PaymentsIndex({ payments, filters, baseCurrency, can }: PaymentsProps) {
    const [search, setSearch] = useState(filters.search);

    const apply = () => {
        router.get('/sales/payments', search === '' ? {} : { search }, {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    return (
        <AppLayout
            ...
            description={`Money in from customers. Amounts in ${baseCurrency} unless a receipt says otherwise.`}
            ...
            actions={
                can.record ? (
                    <Button
                        ...
                        onClick={() => router.get('/sales/payments/new')}
                    >
                        Record a payment
                    </Button>
                ) : undefined
        ...
        >
            ...
            <Card className="mb-4">
                <Input
                    ...
                    placeholder="Receipt number, reference or customer"
            ...
            <Card flush>
                {payments.total === 0 && filters.search === '' ? (
                    <EmptyState
                        ...
                        description="Record money as it arrives. A receipt can settle several invoices, part of one, or nothing at all — in which case it is held as an advance."
                        action={
                            can.record ? (
                                <Button
                                    variant="primary"
                    ...
                    <NoResultsState
                        ...
                        onClear={() => {
                            setSearch('');
                            router.get('/sales/payments', {}, { preserveState: true });
                        }}
                    />
                ) : (
                    <>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="text-content-muted border-line-subtle text-2xs border-b uppercase">
                                        ...
                                        <th className="px-4 py-2 text-left font-medium">Into</th>
