...
import type { DocumentTypeProps } from './Index';
import {
    divideForDisplay,
    formatMoney,
    isPositiveAmount,
 ...
 * The line editor.
 *
 * Keyboard-first, because this is the screen somebody spends an hour a day
 * in: every field is reachable in tab order, Enter on the last line adds
 * another, and choosing an item fills the row from its defaults so the common
 * case is one selection and a quantity.
 *
 * The running totals are display only. The server recomputes every figure
 * through the tax engine at full decimal precision — this side exists so the
 * user is not typing blind, and its arithmetic is never sent back as an
 * amount.
 *
...
export default function DocumentEdit({
        ...
        lines:
            document !== null && document.lines.length > 0
                ? document.lines.map((line) => ({
                      ...
                      revenue_account_id: asText(line.revenue_account_id),
                  }))
                : [{ ...EMPTY_LINE }],
    });
    ...
    /**
     * Choosing an item fills the row from its defaults.
     *
     * Only the fields the user has not already typed into: an item is a
     * starting point, and silently overwriting an edited description is the
     * behaviour people learn to dread.
     */
    ...
    const removeLine = (index: number) => {
        if (form.data.lines.length <= 1) {
            // A document needs a line, so the last one is cleared rather than
            // removed — which is also what the user meant.
    ...
    const chooseCustomer = (contactId: string) => {
        ...
        form.setData((data) => ({
            ...
            due_date:
                data.due_date === '' && customer !== undefined && type.has_due_date
                    ? addDays(data.issue_date, customer.terms)
                    : data.due_date,
        }));
    };
    ...
    const submit = (event: SyntheticEvent<HTMLFormElement>) => {
        ...
        if (isEdit) {
            form.patch(`/sales/${type.segment}/${document.number}`);

            return;
        }

        form.post(`/sales/${type.segment}`);
    };
    ...
    return (
        <AppLayout
            ...
            description={
                type.posts
                    ? 'Saved as a draft. Nothing reaches the ledger until you issue it.'
            ...
            <form onSubmit={submit} className="flex flex-col gap-4">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        {type.has_due_date ? (
                            <Input
                                ...
                                hint="From the customer's terms."
                            />
                        ) : (
                            <Input
                                label="Expires"
                        ...
                        {type.value === 'credit_note' && (
                            <Select
                                ...
                                options={[
                                    { value: '', label: 'Not against a specific invoice' },
                                    ...creditableInvoices,
                                ]}
                                hint="Naming one reduces its balance when this is issued."
                                containerClassName="sm:col-span-2"
                            />
                        )}
                    </div>
                    ...
                    <label className="text-content-secondary mt-4 flex items-start gap-2 text-sm">
                        ...
                        <span>
                            ...
                            <span className="text-content-muted block text-xs">
                                The net is extracted from each price rather than tax being added, so
                                the total equals what you typed.
                            </span>
                        </span>
                    </label>
                </Card>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="flex items-end gap-2">
                                <Input
                                    ...
                                    hint="Spread across the lines, pro rata."
                ...
                <div className="flex items-center justify-end gap-2">
                    <Button
                        variant="ghost"
                        size="md"
                        onClick={() => router.get(`/sales/${type.segment}`)}
                    >
                        Cancel
                    </Button>
                    <Button
 ...
 * date is a database question, and guessing it here would show a figure that
 * the server then contradicts. Better to show the net and say where the rest
 * comes from.
 */
function summarise(
    lines: LineDraft[],
