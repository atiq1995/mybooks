...
/**
 * A list of one kind of sales document.
 *
 * The three figures at the top are the point of the screen: what is
 * outstanding, how much of it is late, and what is still sitting in draft.
 * A list of rows with no summary makes people add up a page to learn what
 * matters, and the answer is usually "chase the overdue column".
 *
 * Overdue is a filter rather than a status, because it changes at midnight
 * without anything happening to the document.
 */
export default function DocumentsIndex({
    ...
    const apply = (next: Partial<typeof filters>) => {
        ...
        router.get(`/sales/${type.segment}`, pruneEmpty(merged), {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    const clear = () => {
        ...
        setDraft(empty);
        router.get(`/sales/${type.segment}`, {}, { preserveState: true, replace: true });
    };
    ...
    return (
        <AppLayout
            ...
            description={
                type.posts
                    ? `Issued ${type.plural.toLowerCase()} post to the ledger. Amounts in ${baseCurrency} unless a document says otherwise.`
                    : `A commitment, not a financial event — a ${type.label.toLowerCase()} never posts to the ledger.`
            }
            breadcrumbs={[{ label: 'Sales' }, { label: type.plural }]}
            actions={
                can.create ? (
                    <Button
                        variant="primary"
                        ...
                        onClick={() => router.get(`/sales/${type.segment}/new`)}
                    >
        ...
        >
            ...
            {type.posts && (
                <div className="mb-4 grid gap-4 sm:grid-cols-3">
                    ...
                    />
                    <Figure label="In draft" value={summary.draft} currency={baseCurrency} />
                </div>
            )}
            ...
            <Card flush>
                {documents.total === 0 && !isFiltered ? (
                    <EmptyState
                        ...
                        description={
                            type.posts
                                ? `Raise one and it becomes a draft. Nothing reaches the ledger until you issue it.`
                                    ...
                                    {documents.data.map((document) => (
                                        ...
                                        >
                                            <td className="px-4 py-2.5">
                                                <Link
                                                    href={`/sales/${type.segment}/${document.number}`}
 ...
 * the reader needs — a "sent" invoice three weeks late is not a sent invoice
 * as far as anybody chasing it is concerned.
 */
function toneOf(document: DocumentSummary): BadgeTone {
...
function Figure({
    ...
    return (
        <Card className="border-line-danger p-0">
            <button
                type="button"
                onClick={onClick}
