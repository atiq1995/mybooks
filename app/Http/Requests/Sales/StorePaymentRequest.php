 ...
 *
 * Shape and arithmetic that the user can fix in the form. Whether an invoice
 * has room for the allocation belongs to the Action, which holds the
...
final class StorePaymentRequest extends FormRequest
{
    ...
    {
        return $this->user()?->can(Permission::SalesRecordPayment->value) ?? false;
    }
        ...
        return [
            'contact_id' => [
                ...
                function (string $attribute, mixed $value, callable $fail): void {
                    if (Contact::query()->usable()->customers()->whereKey(self::asId($value))->first() === null) {
            ...
            'bank_account_id' => [
                ...
                function (string $attribute, mixed $value, callable $fail): void {
                     ...
                     * somebody "receive" money into a revenue account, which
             ...
             * to it. The customer settles the invoice in full and hands part
             * of it to the tax authority on our behalf.
             */
            ...
            'withholding_tax_id' => [
                ...
                'uuid',
                function (string $attribute, mixed $value, callable $fail): void {
                    if (Tax::query()->usable()->withholding()->whereKey(self::asId($value))->first() === null) {
            ...
            'allocations.*.document_id' => [
                ...
                'uuid',
                function (string $attribute, mixed $value, callable $fail): void {
                    if (SalesDocument::query()->whereKey(self::asId($value))->first() === null) {
                        $fail('That invoice could not be found.');
                    }
                },
            ],
        ...
        $validator->after(function (Validator $validator): void {
            ...
            if ($allocated->isGreaterThan($amount)) {
                $validator->errors()->add('allocations', sprintf(
                    ...
                    'the remainder unapplied and it is held as an advance.',
